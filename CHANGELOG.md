...
- ed9e0ee: Refine placeholder interaction so Saurus no longer auto-opens during passive editing, and add placeholder hover actions for opening suggestions or generating with a prompt.
...
- d8544ad: Improve suggestion generation UX by showing the correct loading action state, hiding alternate generation actions while a request is active, and tuning default AI prompt guidance to include more single-word replacements.
